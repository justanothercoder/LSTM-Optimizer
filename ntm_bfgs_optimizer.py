...
class NTM_BFGS_OptimizerLayer(L.layers.MergeLayer):
    def __init__(self, incoming, num_units, function, n_steps, 
        ...
        super(NTM_BFGS_OptimizerLayer, self).__init__(incomings, **kwargs)
        ...
        num_inp = 1 + int(self.use_function_values) + int(self.preprocess_input) + 1
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(theta_previous, read_vector, memory_matrix, input_n, *args):
            input_vector = memory_matrix.dot(read_vector)
            ...
            input_n = T.concatenate([input_n, input_vector.dimshuffle(0, 'x')], axis=1)
            ...
            new_memory = memory_matrix + a.dot(b.T)
            ...
            return [theta, r, new_memory] + new_args
        ...
        memory_init = T.zeros((n_coords, n_coords))
        read_init = T.zeros((n_coords,))
        ...
        scan_out = theano.scan(
            sequences=seqs,
            fn=step_fun,
            outputs_info=[theta_init, None, read_init, memory_init] + inits,
